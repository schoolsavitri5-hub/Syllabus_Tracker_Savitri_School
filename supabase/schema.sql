...
-- The caller sends [{subject, month, chapter, topic, hindi, assessment, practical, project, status, remarks}].
...
begin
  ...
  for r in select value from jsonb_array_elements(import_rows) loop
    ...
    insert into syllabus_topics(subject_id,month,unit_chapter_en,unit_chapter_hi,topic_en,assessment_en,practical,project,status,remarks)
    values(sid,trim(r->>'month'),trim(r->>'chapter'),nullif(r->>'hindi',''),nullif(r->>'topic',''),nullif(r->>'assessment',''),nullif(r->>'practical',''),nullif(r->>'project',''),(r->>'status')::topic_status,nullif(r->>'remarks',''));
  end loop;
end $$;
